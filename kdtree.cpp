...
{
	//dbscan_log << pos[0] << " " << pos[1] << " dir " << dir << " dim " << dim << endl;
...
{
	//dbscan_log << pos[0] << " " << pos[1] << " range " << range << " ordered " << ordered << endl;
...
{
			printf("(%f, %f, %i) ", X, Y, ClusterId);
}
double Point::DistanceSquared(Point p1, Point p2)
{
...
void kd_res_node_data_list(struct res_node *riter)
{
	if (riter){
		Point* g0 = (Point*)riter->item->data;
		points.push_back(g0);
		kd_res_node_data_list(riter->next);
	};
}

std::list<Point*>* kd_data_list(struct kdres *rset)
{
	points.clear();
	kd_res_node_data_list(rset->riter);
	printf("point size: %i\n", points.size());
